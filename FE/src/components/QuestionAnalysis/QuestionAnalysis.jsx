  ...
  return (
    <div className={styles.QuestionAnalysisContainer}>
      ...
      <div className={styles.QuestionAnalysis}>
        ...
        {quizImpression && quizImpression.analysis &&
  Object.keys(quizImpression.analysis).map((quiz, index) => {
    return (
      <div className={styles.QuestionsName} key={index}>
        <h2>Q.{index + 1} {quizImpression.analysis[quiz].question} ?</h2>
        <div className={styles.QuestionsSelected}>
          {quizImpression.questionOrPoll === "Q & A" ? (
            <>
              <p>
                <span>{quizImpression.impression}</span> people attempted the question
              </p>
              <p>
                <span>{quizImpression.analysis[quiz].correct}</span> people answered correctly
              </p>
              <p>
                <span>{quizImpression.analysis[quiz].incorrect}</span> people answered incorrectly
              </p>
            </>
          ) : (
            Object.values(quizImpression.analysis[quiz].selectedOptions).map(
              (impression, index) => (
                <p style={{flexDirection:'row',fontSize:'1vw', width:"23% ",flexGrow:'unset'}} key={index}><span style={{marginRight:"1vw"}} >{impression}</span>{`option ${index + 1}`}</p>
              )
            )
          )}
        </div>
