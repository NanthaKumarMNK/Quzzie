...
export default function Question() {
 ...
 const [finished,setFinished]=useState()
  ...
  useEffect(() => {
    if (question.timer && question.timer !== '0' && timeLeft === 0 && currentQuiz === Object.keys(question.quiz).length - 1) {
      completed();
    }
  ...
  return (
    <div className={styles.QuestionContainer}>
      {!finished && <div className={styles.Question}>
      
        <div className={styles.Timer}>
        <p className={styles.TimerQuestionNumber}>
        {question.quiz &&`0${currentQuiz + 1}/0${Object.keys(question.quiz).length}`}
</p>
         {( !question.timer || question.timer ==='0' )  ? <div></div>:<p className={styles.TimerNumber}>{`00:${timeLeft < 10 ? '0' + timeLeft : timeLeft}s`}</p>
        }
        </div>

        {question.quiz && Object.entries(question.quiz).map(([quizNo, quizData], index) => (
  index === currentQuiz && (
    <div className={styles.quizNo}  key={quizNo}>
      <p className={styles.QuestionName}>{quizData.question} ?</p>
      <div className={styles.Options}>
        {Object.entries(quizData.option).map(([optionKey, optionValue]) => (
          <div className={`${styles.Option} ${selectedOption[quizNo] === optionKey ? styles.SelectedOption : ''}`} onClick={()=>handleClick(quizNo,optionKey)} key={optionKey}>
      <>
          {optionValue.text && <div>{optionValue.text}</div>}
            {optionValue.image && (
              <div
                style={{
  ...
  {finished && finished !=='Poll' &&
     <div className={styles.Completed}>
         ...
         Congrats Quiz is completed
            ...
            <img src={Cup} alt="Cup" />
            <p>Your Score is <span>0{finished}/0{Object.keys(question.quiz).length}</span></p>
          </div> }{finished && finished ==='Poll' &&
         <div className={styles.Completed}> <h1>Thank you
for participating in the Poll</h1></div>}
    </div>
  );
}
